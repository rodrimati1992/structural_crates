...
pub(crate) type TypeParamBounds = Punctuated<syn::TypeParamBound, token::Add>;
...
pub(crate) fn macro_impl(aliases: StructuralAliases) -> Result<TokenStream2, syn::Error> {
    ...
    for saf in list {
        let names_module_definition =
            NamedModuleAndTokens::new(&saf.ident, saf.fields.iter().map(|f| &f.ident));
