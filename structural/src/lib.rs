...
#[derive(Structural)]
enum Command{
    // The `newtype(bound="...")` attribute marks the variant as being a newtype variant,
    ...
    #[struc(newtype(bound="SendEmail_VSI<@variant>"))]
    SendEmail(SendEmail),
...
For a macro to declare a structural type directly as a pareter or return type
