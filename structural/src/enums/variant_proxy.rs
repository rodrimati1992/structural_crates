...
///
/// ```
/// use structural::{fp,ts,GetFieldExt,Structural,TS};
...
/// /// `this.is_variant(ts!(Bar))` (the method is from `GetFieldExt`) must return true.
...
impl<T: ?Sized, V> VariantProxy<T, TStr<V>> {
    ...
    ///
    /// ```
    /// use structural::{fp,ts,TS,field_path_aliases,GetFieldExt,Structural};
    /// use structural::enums::VariantProxy;
    ///
